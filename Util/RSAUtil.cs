...
namespace Aop.Api.Util
{
    ...
    {
        public static RSA LoadCertificateFile(string filename, string signType)
        {
            using (System.IO.FileStream fs = System.IO.File.OpenRead(filename))
            {
                ...
                try
                {
                    RSA rsa = CreateRsaFromPrivateKey(res, signType);
                    return rsa;
                }
                catch (Exception ex)
                {
        ...
        public static RSA LoadCertificateString(string strKey, string signType)
        {
            try
            {
                var privateKeyBits = Convert.FromBase64String(strKey);
                RSA rsa = CreateRsaFromPrivateKey(privateKeyBits, signType);
                return rsa;
            }
            catch (Exception ex)
            {
        ...
        private static RSA CreateRsaFromPrivateKey(byte[] bytes, string signType)
        {
            int bitLen = 1024;
