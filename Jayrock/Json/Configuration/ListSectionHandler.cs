...
namespace Jayrock.Configuration
{
    ...
    public abstract class ListSectionHandler
    {
        ...
        public virtual object Create(object parent, object configContext, XmlNode section)
        {
            ...
            foreach (XmlNode childNode in section.ChildNodes)
            {
                ...
                {
                    throw new Exception(string.Format("Unexpected type of node ({0}) in configuration.", 
                        childNode.NodeType.ToString()));
                }

                if (childNode.Name != itemName)
                {
                    throw new Exception(string.Format("Element <{0}> is not valid here in configuration. Use <{1}> elements only.", 
